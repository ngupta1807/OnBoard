...
public class ListAdapter extends BaseAdapter {
    ...
    public View getView(int i, View mView, ViewGroup viewGroup) {
        ...
        if (deliveryData.getItemPhoto() != null && !deliveryData.getItemPhoto().equals("") && !deliveryData.getItemPhoto().contentEquals("null"))
            Picasso.with(this.ctx).load(deliveryData.getItemPhoto()).into(viewHolder.itemImg);
        ...
        if (deliveryData.getDoBuildType().equals("1")) {
            viewHolder.dtype.setText("Commercial");
            viewHolder.dtype.setCompoundDrawablesWithIntrinsicBounds(R.drawable.commerical_icon_green, 0, 0, 0);
        } else {
            viewHolder.dtype.setText("Residential");
            viewHolder.dtype.setCompoundDrawablesWithIntrinsicBounds(R.drawable.residental_icon_green, 0, 0, 0);
        }
       ...
       /* if (deliveryData.getDeliveryStatus().equalsIgnoreCase("0")) {
        ...
        if (deliveryData.getAllocateDriverID() != null && deliveryData.getAllocateDriverID().equalsIgnoreCase(session.getUserDetails().getId())) {
            viewHolder.bidPrice.setText("ALLOCATED DELIVERY");
            viewHolder.bid.setText("");
        } else if (!deliveryData.getMaxOpeningBid().equals("null")) {
