...
public class AddLink extends Activity implements OnClickListener,
	...
	public void onClick(View v) {
		...
		case R.id.next: {
			Intent intent = new Intent(AddLink.this, ListCategory.class);
			startActivity(intent);
			finish();
		}
			break;
		case R.id.delete: {
			CustomAlertDialog cam = new CustomAlertDialog(AddLink.this,
					AddLink.this);
		...
		case R.id.back:
			if (spm.getStringValues("menu").equals("click")) {
				if (spm.getStringValues("level").equals("level1")) {
					...
					finish();
					Intent intent = new Intent(AddLink.this,
				...
				if (getIntent().getStringExtra("title").equals("Add")) {
					...
					Intent intent = new Intent(AddLink.this, SelectLevel.class);
					intent.putExtra("selectedvalue", getIntent()
							.getStringExtra("selectedvalue"));
					intent.putExtra("title", getIntent()
							.getStringExtra("title"));
	...
	private void selectImage() {
		 ...
		 */
		CustomAlertDialog cad = new CustomAlertDialog(mcon, AddLink.this,
				"image");
			...
			if (requestCode == REQUEST_TAKE_PHOTO) {
				if (ViewClicked == 0) {
					...
					new ImageCompressing().saveImage(pic_path.getText()
							.toString(), bit, AddLink.this, file_name);
	...
	private String createDirectoryAndSaveFile(Bitmap imageToSave,
		...
		if (!direct.exists()) {
			File Directory = new File(
					new GetPathForImage().getBasePathForCopy(AddLink.this));
	...
	private void appendEditableItems() {
		if (!getIntent().getExtras().getString("itemid").equals("0")) {
			LinkModel scm;
			ArrayList<LinkModel> devicelist = new GetDataFrmDB()
					.getAllLinkItemResult(
							mDbHelper,
							Integer.parseInt(getIntent().getStringExtra(
									"itemid")));
			for (int i = 0; i < devicelist.size(); i++) {
				scm = (LinkModel) devicelist.get(i);

				ed_sub_cat_item_name.append(scm.getKEY_LINK_BODY());

				pic_path.setText(scm.getKEY_LINK_PIC());

				if (scm.getKEY_LINK_LINK().contains("?:")) {
					System.out.println("Link split:..."
							+ scm.getKEY_LINK_LINK());
					String lk2[] = scm.getKEY_LINK_LINK().toString()
							.split(Pattern.quote("?:"));
					link.setText(lk2[0]);
					link2.setVisibility(View.VISIBLE);
					link2.setText(lk2[1]);
				} else {
					link.setText(scm.getKEY_LINK_LINK());
				}
				data.setText(scm.getKEY_LINK_DATA());
				Boolean status=Boolean.valueOf(scm.getkEY_LINK_STATUS());
	...
	public void onBackPressed() {
		...
		if (spm.getStringValues("menu").equals("click")) {
			...
			if (getIntent().getStringExtra("title").equals("Add")) {
				Intent intent = new Intent(AddLink.this, SelectCategory.class);
	...
	public void selected(String action) {
		if (action.equals("delete")) {
			...
			if (spm.getStringValues("level").equals("level1")) {
				...
				Intent intent = new Intent(AddLink.this, ListSubCategory.class);
				startActivity(intent);
				finish();
			}
		} else {
		...
		if (cat_id.equals("0") || cat_id.equals("")) {
			...
			if (!getIntent().getExtras().getString("itemid").equals("0")) {
				...
				if (link2.getText().toString().equals("")) {
					mDbHelper.updateLinkItem(id, catid, path,
				...
				if (link2.getText().toString().equals("")) {
					mDbHelper.createLinkItem(catid, path, ed_sub_cat_item_name
