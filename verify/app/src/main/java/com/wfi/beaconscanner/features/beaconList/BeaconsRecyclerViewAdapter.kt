...
import com.wfi.beaconscanner.utils.CountHelper
...
class BeaconsRecyclerViewAdapter(val data: RealmResults<BeaconSaved>,
    ...
    class EddystoneUidHolder(itemView: View, ctx: Context, listener: OnControlsOpen?) : BaseHolder(itemView, ctx, listener) {
        ...
        @BindView(R.id.beacon_id) lateinit var beaconID: TextView
        @BindView(R.id.beacon_id_icon) lateinit var beaconIDImage: ImageView
        ...
        override fun bindView(beacon: BeaconSaved) {
            ...
            if(beacon.sStatus.equals("1")){
                cardView.setCardBackgroundColor(ContextCompat.getColor(ctx, R.color.colorPauseFab))
                beaconType.text = String.format(Locale.getDefault(), "%s%s",
                        itemView.context.getString(R.string.eddystone_off),
                        if (beacon.telemetryData != null) "" else "")
            }else {
            ...
            if (eddystoneUidData != null) {
                ...
                if(eddystoneUidData.instanceId!!.startsWith("0x"))
                        instanceId.text = String.format(ctx.getString(R.string.instance_id_x), eddystoneUidData.instanceId!!.substring(2,eddystoneUidData.instanceId!!.length))
                else
                    instanceId.text = String.format(ctx.getString(R.string.instance_id_x), eddystoneUidData.instanceId)

                if(beacon.beaconName.contains("na") || beacon.beaconName.length==0) {
                    beaconID.visibility = View.VISIBLE
                    beaconIDImage.visibility = View.VISIBLE
                    beaconID.text = String.format(ctx.getString(R.string.beacon_id_x),"")
                }else{
                    beaconID.visibility = View.VISIBLE
                    beaconIDImage.visibility = View.VISIBLE
                    beaconID.text = String.format(ctx.getString(R.string.beacon_id_x), beacon.beaconName.replace("\"", ""))
                }
            }
        }
    }
    ...
    class OfflineUidHolder(itemView: View, ctx: Context, listener: OnControlsOpen?) : BaseHolder(itemView, ctx, listener) {
        @BindView(R.id.namespace_id) lateinit var namespaceId: TextView
        @BindView(R.id.instance_id) lateinit var instanceId: TextView
        @BindView(R.id.beacon_id) lateinit var beaconID: TextView
        @BindView(R.id.beacon_id_icon) lateinit var beaconIDImage: ImageView
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        when (b?.beaconType) {
            ...
            BeaconSaved.TYPE_OFFLINE -> return R.layout.eddystone_uid_item
